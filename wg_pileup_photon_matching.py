...
histogram_models = [ROOT.RDF.TH1DModel('', '', n_photon_pt_bins, binning_photon_pt ),ROOT.RDF.TH1DModel('','',16,0,6),ROOT.RDF.TH1DModel('','',16,0,pi),ROOT.RDF.TH1DModel('','',48,0,pi), ROOT.RDF.TH1DModel("met", "", 15 , 0., 300 ), ROOT.RDF.TH1DModel('lepton_pt', '', 15, 25., 100 ), ROOT.RDF.TH1DModel('lepton_eta', '', 10, -2.5, 2.5 ), ROOT.RDF.TH1DModel('', '', n_photon_pt_bins, binning_photon_pt ), ROOT.RDF.TH1DModel('photon_eta', '', 10, -2.5, 2.5 ), ROOT.RDF.TH1DModel("mlg","",mlg_fit_upper_bound/2,0,mlg_fit_upper_bound), ROOT.RDF.TH1DModel("mlg","",100,0,200),ROOT.RDF.TH1DModel("lepton_phi","",14,-3.5,3.5), ROOT.RDF.TH1DModel("photon_phi","",14,-3.5,3.5), ROOT.RDF.TH1DModel("njets40","",7,-0.5,6.5), ROOT.RDF.TH1DModel("mt","",10,0,200), ROOT.RDF.TH1DModel("puppimt","",10,0,200), ROOT.RDF.TH1DModel("npvs","",51,-0.5,50.5), ROOT.RDF.TH1DModel("drlg","",16,0,5), ROOT.RDF.TH1DModel('photon_pt', '', 25, 25., 75 ),ROOT.RDF.TH1DModel("met", "", 15 , 0., 300 ),ROOT.RDF.TH1DModel('photon_recoil', '', 20, -70., 130 )] 
...
xpositions = [0.28,0.28,0.28,0.28]
ypositions = [0,1,2,3]
...
wjets_pileup_genjet = {}
wjets_pileup_mergedgen = {}


wjets_pileup_genjet["hists"] = []
wjets_pileup_mergedgen["hists"] = []
...
for i in range(len(variables)):
    wjets_pileup_genjet["hists"].append(histogram_models[i].GetHistogram())
    wjets_pileup_mergedgen["hists"].append(histogram_models[i].GetHistogram())

for i in range(len(variables)):
    wjets_pileup_genjet["hists"][i].Sumw2()
    wjets_pileup_genjet["hists"][i].SetName("wjets pileup genjet"+variables[i])
    wjets_pileup_mergedgen["hists"][i].Sumw2()
    wjets_pileup_mergedgen["hists"][i].SetName("wjets pileup mergedgen "+variables[i])
...
rinterface = rinterface.Define("weight_pileup_genjet","(photon_genjet_matching == 0 && photon_selection == 0 && is_lepton_tight == 1 && is_lepton_real == 1)*base_weight")
rinterface = rinterface.Define("weight_pileup_mergedgen","(photon_mergedgen_matching < 5 && photon_selection == 0 && is_lepton_tight == 1 && is_lepton_real == 1)*base_weight")

for variable_definition in variable_definitions:
    rinterface = rinterface.Define(variable_definition[0],variable_definition[1])
    
rresultptrs_wjets_pileup_genjet = []    
rresultptrs_wjets_pileup_mergedgen = []    

for i in range(len(variables)):
    rresultptrs_wjets_pileup_genjet.append(rinterface.Histo1D(histogram_models[i],variables[i],"weight_pileup_genjet"))
    rresultptrs_wjets_pileup_mergedgen.append(rinterface.Histo1D(histogram_models[i],variables[i],"weight_pileup_mergedgen"))
    

for i in range(len(variables)):
    wjets_pileup_genjet["hists"][i].Add(rresultptrs_wjets_pileup_genjet[i].GetValue())
    wjets_pileup_mergedgen["hists"][i].Add(rresultptrs_wjets_pileup_mergedgen[i].GetValue())

for i in range(len(variables)):
    wjets_pileup_genjet["hists"][i].SetLineColor(ROOT.kRed)
    wjets_pileup_mergedgen["hists"][i].SetLineColor(ROOT.kBlue)
    wjets_pileup_genjet["hists"][i].SetLineWidth(2)
    wjets_pileup_mergedgen["hists"][i].SetLineWidth(2)
...
for i in range(len(variables)):
    ...
    set_axis_fonts(wjets_pileup_genjet["hists"][i],"x",getXaxisLabel(variables[i]))

    wjets_pileup_genjet["hists"][i].SetMinimum(0)
    wjets_pileup_genjet["hists"][i].SetMaximum(1.55*max(wjets_pileup_genjet["hists"][i].GetMaximum(),wjets_pileup_genjet["hists"][i].GetMaximum()))

    wjets_pileup_genjet["hists"][i].Draw()
    wjets_pileup_mergedgen["hists"][i].Draw("same")
    ...
    j=0
    draw_legend(xpositions[j],0.84 - ypositions[j]*yoffset,wjets_pileup_genjet["hists"][i],"genjet non-matched photons","l")
    j=1
    draw_legend(xpositions[j],0.84 - ypositions[j]*yoffset,wjets_pileup_mergedgen["hists"][i],"genpart non-matched photons","l")
